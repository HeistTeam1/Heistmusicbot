...
async def process_play_command(message: Message, query: str):
    ...
    if isinstance(result, dict) and "playlist" in result:
        ...
        if secs > MAX_DURATION_SECONDS:
            await processing_message.edit(
                "❌ Streams longer than 15 min are not allowed. If u are the owner of this bot contact @Assaulted_dark to upgrade your plan"
            )
            return
...
async def frozen_check_loop(bot_username: str):
    while True:
        try:
            ...
            await assistant.send_message(bot_username, "/heist_check")
            ...
            while time.time() < deadline:
                async for msg in assistant.get_chat_history(bot_username, limit=1):
                    ...
                    if "frozen check successful ✨" in text.lower():
                        got_ok = True
                        logger.info("Received heist check confirmation.")
                        break
                if got_ok:
                    break
                await asyncio.sleep(3)
